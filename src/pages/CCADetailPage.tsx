  ...
  const toggleWishlist = useMutation({
    mutationFn: async () => {
      ...
      if (isWishlisted) {
        ...
        await supabase
          .from("cca_wishlist")
          .insert({ cca_id: id!, user_id: user.id });
      }
    },
    onSuccess: () => {
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 container mx-auto">
          <h1 className="font-anton text-4xl md:text-5xl text-primary-foreground italic mb-2">
            {cca.name}
          </h1>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {cca.tags?.map((tag) => (
              <Badge
                key={tag}
                className="bg-card/20 text-primary-foreground border-none font-montserrat"
              >
                {tag}
              </Badge>
            ))}
